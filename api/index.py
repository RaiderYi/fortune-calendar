...
class handler(BaseHTTPRequestHandler):
    ...
    def _handle_ai_chat(self):
        ...
        try:
            ...
            # 获取参数
            messages = data.get('messages', [])  # 聊天历史
            bazi_context = data.get('baziContext', {})  # 八字上下文
            
            # 构建系统提示词
            system_prompt = self._build_bazi_system_prompt(bazi_context)
            
            # 调用 DeepSeek API
            api_key = os.environ.get('DEEPSEEK_API_KEY', 'sk-5de6598eb59e448f9e829350d56cbd00')
            if not api_key:
            ...
            # 调用 DeepSeek API
            response_text = self._call_deepseek_api(api_key, api_messages)
    ...
    def _build_bazi_system_prompt(self, context):
        """构建包含八字信息的系统提示词"""
